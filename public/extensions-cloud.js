...
function cleanupUninstalledDeletedExtensions() {
  ...
  if (cleaned.length !== installed.length) saveInstalledExtensions(cleaned);
}
...
async function trackExtensionInstall(extId, installed) {
  try {
    ...
    if (installed) {
      await setDoc(doc(db, EXT_INSTALLS_COLLECTION, docId), { extensionId: extId, sessionId: sessId, installedAt: Date.now() });
    } else {
...
async function loadAdminExtensionsTab() {
  ...
  let html = `
    ...
    <div class="adm-form" style="max-width:600px;">
      ...
      <div class="adm-field"><label>SHORT DESCRIPTION *</label><input class="adm-input" id="ext-desc" placeholder="One-line description"></div>
      <div class="adm-field"><label>LONG DESCRIPTION</label><textarea class="adm-textarea" id="ext-longdesc" rows="2"></textarea></div>
      <div class="adm-field"><label>HOW TO USE</label><textarea class="adm-textarea" id="ext-howto" rows="2"></textarea></div>
      <div class="adm-field"><label>EXAMPLE</label><textarea class="adm-textarea" id="ext-example" rows="2"></textarea></div>
      <div class="adm-field">
        <label>TYPE *</label>
      ...
      <div id="ext-type-tool-fields">
        <div class="adm-field">
          ...
          <textarea class="adm-textarea" id="ext-code" rows="8" style="font-family:monospace;font-size:11px;" placeholder="return text.toUpperCase();"></textarea>
        </div>
      </div>
      <div id="ext-type-theme-fields" style="display:none;">
        <div class="adm-field"><label>BASE THEME</label>
          <select class="adm-input" id="ext-theme-base"><option value="vs-dark">vs-dark</option><option value="vs">vs (light)</option><option value="hc-black">hc-black</option></select>
        </div>
        <div class="adm-field"><label>BACKGROUND</label><input class="adm-input" type="color" id="ext-theme-bg" value="#1e1e1e"></div>
        <div class="adm-field"><label>FOREGROUND</label><input class="adm-input" type="color" id="ext-theme-fg" value="#d4d4d4"></div>
      ...
      <div id="ext-type-snippet-fields" style="display:none;">
        <div class="adm-field">
          ...
          <textarea class="adm-textarea" id="ext-snippets" rows="6" style="font-family:monospace;font-size:11px;" placeholder='[{"name":"Name","desc":"Desc","code":"console.log(1);"}]'></textarea>
        </div>
      </div>
    ...
    <div class="adm-section-title" style="margin-top:24px;">// BUILT-IN PACKS (${Object.keys(allBuiltIn).length})</div>
...
function renderExtLeaderboard(stats, allBuiltIn) {
  ...
  return `<div class="adm-history-list">` + sorted.map(([id,count]) => {
    ...
    return `<div class="adm-hist-card" style="display:flex;align-items:center;gap:10px;">
      <span style="font-size:18px;">${ext?ext.icon:"❓"}</span>
      <span style="flex:1;">${ext?ext.name:id+" (deleted)"}</span>
...
function renderAdminBuiltinList(allBuiltIn, stats) {
  ...
  return `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:6px;">` +
    ids.map(id => {
...
async function adminSubmitExtension() {
  ...
  if (type === "tool") {
    ...
    if (!data.code.trim()) { if(status){status.innerText="⚠ JS code required";status.style.color="#ffaa00";} return; }
  } else if (type === "theme") {
    data.themeConfig = {
    ...
    } catch { if(status){status.innerText="⚠ Invalid JSON array";status.style.color="#ffaa00";} return; }
  }
  if (status) { status.innerText = "// Saving..."; status.style.color="#00d4ff"; }
  const id = await adminSaveExtension(data);
  if (id) {
    if (status) { status.innerText = "✅ Extension added!"; status.style.color="#00ff88"; }
...
async function adminDeleteExtensionConfirm(id) {
  const ext = cloudExtensions[id];
  if (!confirm(`Delete "${ext?.name||id}"? Removes for ALL users.`)) return;
...
function runCloudExtensionTool(id) {
  ...
  try {
    ...
    if (typeof result === "string") { replaceEditorText(ed, sel, result); showToast(ext.name + " applied ✓", "success"); }
  } catch(e) { showToast("Extension error: " + e.message, "error"); }
}
...
function applyCloudExtensionTheme(id) {
  ...
  monaco.editor.defineTheme(themeName, {
    base: c.base || "vs-dark", inherit: true,
    rules: [
  ...
  overlay.innerHTML = `
    ...
    <div class="snippet-menu">
      ...
      <div class="snippet-menu-list">
        ${ext.snippets.map(s=>`<div class="snippet-item" onclick="insertSnippetCode(${JSON.stringify(s.code).replace(/"/g,'&quot;')})">
